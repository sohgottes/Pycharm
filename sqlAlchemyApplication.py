...
class User(Base):
    ...
    def __repr__(self):
        return "User(id={self.id}, name={self.name}, fullname={self.fullname})"
...
class Address(Base):
    ...
    def __repr__(self):
        return "Address(id={self.id}, email_address={self.email_address})"
...
with Session(engine) as session:
    ...
    sandy = User(
        name='Junior',
        fullname='Junior Cardoso',
        address=[Address(email_address='junior@email.br'),
                 Address(email_address='junior@email.org')]
    )
...
stmt = select(User).where(User.name.in_(["guilherme", 'junior']))
